  ...
  return (
    <ScreenContainer className="px-0">
      ...
      <View className="flex-row items-center justify-between px-6 py-4 border-b border-gray-100">
        ...
        <Text className="text-lg font-bold text-textPrimary">Trip History</Text>
        <Pressable
          onPress={() => refetch()}
          className="h-10 w-10 items-center justify-center rounded-xl bg-gray-100 active:bg-gray-200"
        >
      ...
      {isLoading ? (
        ...
        <View className="flex-1 items-center justify-center px-6 gap-2">
          ...
          <Text className="text-base font-bold text-textPrimary">No trips yet</Text>
          <Text className="text-sm text-gray-500 text-center">
            You haven't completed any trips yet. Go online to start receiving bookings!
          </Text>
        </View>
      ) : (
        <FlatList
                ...
                <View className="flex-row justify-between items-center border-t border-gray-50 pt-3">
                  <View className="flex-row gap-2">
                    ...
                    <View className="bg-green-50 px-2 py-0.5 rounded">
                      <Text className="text-[10px] text-driver font-bold uppercase">{item.paymentStatus.replace('_', ' ')}</Text>
                    </View>
                  </View>
                  <View className="items-end">
                    <Text className="text-xs text-gray-400">Your Share</Text>
                    <Text className="font-bold text-driver">₹{formatMoney(item.driverFee)}</Text>
                  </View>
                </View>
              </View>
            );
